...
import { updateBitacoraSubcategoria } from "@/actions";
import { useRouter } from "next/navigation";


interface UpdateModalSubcategoriasProps {
    IdSubcategoria: number;
    onGuardado: () => void;
}

//Guarda la informacion del Subcategoria
...
function UpdateModalSubcategorias({ IdSubcategoria, onGuardado }: UpdateModalSubcategoriasProps) {
    ...
    const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => {
        ...
        if (String(Subcategoria![name as keyof Subcategoria]) !== value) {
            setBitacora((prev) => ({
                ...prev,
    ...
    //Funcion para obtener el id del Subcategoria
    const getSubcategoria = async () => {
        const response = await axios.get(`/api/users/administrador/productos/subcategorias/${IdSubcategoria}`);
    ...
    const handleSubmit = async () => {
        ...
        if (Object.keys(newErrors).length == 0) {
            ...
            const registrosBitacoras: Bitacora[] = Object.entries(bitacora!).map(([key, value]) => ({
                Campo: key,
                ValorAnterior: Subcategoria ? key == 'Tipo' ? Subcategoria['NombreTipo'] : Subcategoria[key as keyof Subcategoria] : "",
                ValorNuevo: value,
            }));
            console.log(registrosBitacoras);
            console.log(inputValue);
            const response = await updateBitacoraSubcategoria(IdSubcategoria, registrosBitacoras);
            const responseUpdate = await axios.put(`/api/users/administrador/productos/subcategorias/${IdSubcategoria}`, inputValue)
                        ...
                        <div className="bg-white p-6 rounded-lg shadow-lg max-w-lg w-full max-h-[90vh] h-auto overflow-y-auto">
                            ...
                            <div className="flex justify-center flex-col items-center py-3">
                                ...
                                <div className=" w-full mt-3">
                                    ...
                                    <select onChange={handleChange} name="Tipo" defaultValue={Subcategoria?.Tipo} className={`flex-grow border rounded-md w-full py-3 px-2 ${errors["Tipo"] ? "border-red-500" : "border-black"}`}>
                                ...
                                <div className="w-full mt-3">
                                    ...
                                    <input
                                        ...
                                        name="CostoBase"
                                        defaultValue={Subcategoria?.CostoBase}
                                        onChange={handleChange}
                                    />
...
export default UpdateModalSubcategorias;
