...
export async function DELETE(request, { params }) {
    ...
    try {
        console.log(params);
        const [response] = await conn.query("CALL SP_DELETECLIENTE(?,?)", [params.id, params.user]);
...
export async function PUT(request, { params }) {
    ...
    try {
        const [response] = await conn.query("CALL SP_REGISTRARACTUZALICACION(?,?,?,?,?,?)", [user, 'Cliente', id, req.Campo, req.ValorAnterior, req.ValorNuevo]);
