...
interface VentaVendedorProps {
    IdClienteProp: string;
}

function VentaVendedor({ IdClienteProp }: VentaVendedorProps) {
    ...
    useEffect(() => {
        getNumeroNotaVenta();
        setClienteSeleccionado(IdClienteProp ? nombresClientes.find((cliente) => Number(cliente.Id) == Number(IdClienteProp)) || null : null);
    }, [nombresClientes])
