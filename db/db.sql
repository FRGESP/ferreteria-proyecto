...
CREATE TABLE PEDIDO(
    ...
    CONSTRAINT FK_PEDIDOTONOTA FOREIGN KEY(IdNota) REFERENCES NOTA(IdNota),
    CONSTRAINT FK_PEDIDOTOSUCURSAL FOREIGN KEY(IdSucursal) REFERENCES SUCURSAL(IdSucursal)
);
...
CREATE PROCEDURE SP_GETSUCURSALES(IN NOMBREIN VARCHAR(100))
    BEGIN
        SELECT S.IdSucursal, S.Nombre, (SELECT FN_OBTENERDIRECCION(S.IdDireccion)) AS Direccion, S.Telefono, S.FechaRegistro  FROM SUCURSAL AS S WHERE S.Nombre LIKE CONCAT('%',NOMBREIN,'%');
    end;
...
DROP FUNCTION IF EXISTS FN_CALCULARTOTAL;
CREATE FUNCTION FN_CALCULARTOTAL(IDNOTAIN INT, IDRANGOCL INT, USERID INT, IDCL INT)
RETURNS DECIMAL(65,2)
    DETERMINISTIC
    BEGIN
        DECLARE TOTAL DECIMAL(65,30);
        ...
        IF DISTANCIA <= RANGODIST THEN
            SET COSTOENVIO = 0;
            ELSE
                SET COSTOENVIO = (CEIL((DISTANCIA-RANGODIST)*CARGODIST));
        end if;

        SET TOTAL = (SELECT SUM((SELECT FN_GETPRECIO(NP.IdProducto,IDRANGOCL)*NP.Piezas)) FROM NOTA_PRODUCTOS AS NP WHERE NP.IdNota = IDNOTAIN);
        IF TOTAL IS NULL THEN
            SET TOTAL = 0;
        end if;
        SET TOTAL = (TOTAL+COSTOENVIO);
        RETURN TOTAL;
    end;
...
CREATE PROCEDURE SP_ADDPRODUCTOVENTA(IN IDPROD INT, IN IDCLIENTEIN INT, IN USERID INT, IN PIEZASIN INT, IN NOTAIN INT)
    BEGIN
        ...
        DECLARE IDPRODVAR INT;
        DECLARE INVENTARIOSUCACTUAL INT;
        DECLARE PIEZASNOTA INT;
        DECLARE PIEZASINSERT INT;
        ...
        SET INVENTARIOSUCACTUAL = (SELECT I.Stock FROM INVENTARIO_SUCURSAL AS I INNER JOIN EMPLEADO E2 on I.IdSucursal = E2.IdSucursal WHERE I.IdProducto = IDPROD AND I.Estatus != 3 AND I.IdSucursal = E2.IdSucursal AND E2.IdEmpleado = EMPLEADOVAR AND I.Estatus = 1 LIMIT 1);
        IF INVENTARIOSUCACTUAL IS NULL OR INVENTARIOSUCACTUAL = 0 THEN
            SELECT -1 AS Error;
        ELSE
            SET IDPRODVAR = (SELECT IdNotaProducto FROM NOTA_PRODUCTOS WHERE IdNota = NOTAVAR AND IdProducto = IDPROD LIMIT 1);

        IF IDPRODVAR IS NULL THEN
            SET PIEZASNOTA = 0;
        ELSE
            SET PIEZASNOTA = (SELECT Piezas FROM NOTA_PRODUCTOS WHERE IdNotaProducto = IDPRODVAR);
        end if;

        IF INVENTARIOSUCACTUAL <= (PIEZASNOTA+PIEZASIN) THEN
            SET PIEZASINSERT = INVENTARIOSUCACTUAL;
            SET PIEZASNOTA = 0;
            SELECT IDPROD AS Error;
        ELSE
            SET PIEZASINSERT = PIEZASIN;
            SELECT 0 AS Error;
        end if;

        IF IDPRODVAR IS NULL THEN
            INSERT INTO NOTA_PRODUCTOS (IdNota, IdProducto, Piezas) VALUES (NOTAVAR, IDPROD, PIEZASINSERT);
        ELSE
            UPDATE NOTA_PRODUCTOS SET Piezas = (PIEZASINSERT+PIEZASNOTA) WHERE IdNota = NOTAVAR AND IdProducto = IDPROD;
        end if;
        end if;
    end;

DROP PROCEDURE IF EXISTS SP_CANCELARPEDIDO;
CREATE PROCEDURE SP_CANCELARPEDIDO(IN IDNOTAIN INT)
    BEGIN
        DELETE FROM NOTA_PRODUCTOS WHERE IdNota = IDNOTAIN;
        DELETE FROM NOTA WHERE IdNota = IDNOTAIN;
    end;

DROP PROCEDURE IF EXISTS SP_GETNOTA;
CREATE PROCEDURE SP_GETNOTA(IN IDNOTAIN INT)
    BEGIN
        DECLARE CLIENTERANGOVAR INT;
        DECLARE USERVAR INT;
        DECLARE IDCLIENTEVAR INT;
        DECLARE NUMPRODUCTOS INT;
        SET NUMPRODUCTOS = (SELECT SUM(Piezas) FROM NOTA_PRODUCTOS WHERE IdNota = IDNOTAIN);
        SET IDCLIENTEVAR = (SELECT N.IdCliente FROM NOTA AS N WHERE N.IdNota = IDNOTAIN LIMIT 1);
        SET USERVAR = (SELECT U.IdUsuario FROM NOTA AS N INNER JOIN EMPLEADO E on N.IdEmpleado = E.IdEmpleado INNER JOIN USUARIO U on E.IdEmpleado = U.IdEmpleado WHERE N.IdNota = IDNOTAIN LIMIT 1);
        SET CLIENTERANGOVAR = (SELECT C.IdRangoCliente FROM CLIENTE AS C INNER JOIN NOTA N2 on C.IdCliente = N2.IdCliente WHERE N2.IdNota = IDNOTAIN LIMIT 1);
        SELECT NP.IdNotaProducto, P.IdProducto, I.Stock ,P.Descripcion, NP.Piezas, FORMAT((SELECT FN_GETPRECIO(P.IdProducto, CLIENTERANGOVAR)),2) AS PrecioUnitario, FORMAT(((SELECT FN_GETPRECIO(P.IdProducto, CLIENTERANGOVAR))*NP.Piezas),2) AS Importe FROM NOTA_PRODUCTOS AS NP INNER JOIN PRODUCTO AS P ON NP.IdProducto = P.IdProducto INNER JOIN NOTA N on NP.IdNota = N.IdNota INNER JOIN INVENTARIO_SUCURSAL I on P.IdProducto = I.IdProducto INNER JOIN EMPLEADO E2 on N.IdEmpleado = E2.IdEmpleado WHERE NP.IdNota = IDNOTAIN AND I.Estatus != 3 AND I.IdProducto = P.IdProducto AND I.IdSucursal = E2.IdSucursal;
        SELECT FN_CALCULARTOTAL(IDNOTAIN, CLIENTERANGOVAR, USERVAR, IDCLIENTEVAR) AS Total;
        SELECT NUMPRODUCTOS AS NumeroProductos;
        CALL SP_CALCULARENVIO(IDCLIENTEVAR,USERVAR);
        SELECT IdCliente FROM NOTA WHERE IdNota = IDNOTAIN;
    end;

DROP PROCEDURE IF EXISTS SP_DELETENOTAPRODUCTO;
CREATE PROCEDURE SP_DELETENOTAPRODUCTO(IN IDNP INT)
    BEGIN
        DELETE FROM NOTA_PRODUCTOS WHERE IdNotaProducto = IDNP;
    end;

DROP PROCEDURE IF EXISTS SP_PAGOEFECTIVO;
CREATE PROCEDURE SP_PAGOEFECTIVO(IN MONTOIN DECIMAL(65,30))
    BEGIN
        INSERT INTO EFECTIVO (Monto) VALUES (MONTOIN);
        SELECT LAST_INSERT_ID() AS IdPago;
    end;

DROP PROCEDURE IF EXISTS SP_PAGOTRANSFERENCIA;
CREATE PROCEDURE SP_PAGOTRANSFERENCIA(IN TITULARIN VARCHAR(100), IN BANCOIN VARCHAR(50), IN CONCEPTOIN VARCHAR(300), IN MONTOIN DECIMAL(65,30))
    BEGIN
        INSERT INTO TRANSFERENCIA (NombreTitular, BancoOrigen, Concepto, Monto) VALUES (TITULARIN, BANCOIN, CONCEPTOIN, MONTOIN);
        SELECT LAST_INSERT_ID() AS IdPago;
    end;

DROP PROCEDURE IF EXISTS SP_PAGOCHEQUE;
CREATE PROCEDURE SP_PAGOCHEQUE(IN TITULARIN VARCHAR(100), IN BANCOIN VARCHAR(50), IN NUMEROCHEQUEIN VARCHAR(20), IN MONTOIN DECIMAL(65,30))
    BEGIN
        INSERT INTO CHEQUE (NombreEmisor, BancoEmisor, NumeroCheque, Monto) VALUES (TITULARIN, BANCOIN, NUMEROCHEQUEIN, MONTOIN);
        SELECT LAST_INSERT_ID() AS IdPago;
    end;

DROP PROCEDURE IF EXISTS SP_CONSULTARCREDITO;
CREATE PROCEDURE SP_CONSULTARCREDITO(IN IDCL INT, IN MONTOIN DECIMAL(65,30))
    BEGIN
        DECLARE DEUDA DECIMAL(65,30);
        DECLARE CREDITOMAX DECIMAL(65,30);

        SET CREDITOMAX = (SELECT C.CreditoMaximo FROM CLIENTE AS C WHERE C.IdCliente = IDCL LIMIT 1);
...
DROP FUNCTION IF EXISTS FN_ACTUALIZARSTOCK;
CREATE FUNCTION FN_ACTUALIZARSTOCK(IDNOTAIN INT, IDPROD INT, IDSUC INT, USERID INT)
RETURNS VARCHAR(10)
    DETERMINISTIC
    BEGIN
        DECLARE STATUSVAR VARCHAR(10);
        DECLARE PIEZASVAR INT;
        DECLARE PRODRESER INT;
        DECLARE STOCKACTUAL INT;

        SET PIEZASVAR = (SELECT Piezas FROM NOTA_PRODUCTOS WHERE IdNota = IDNOTAIN AND IdProducto = IDPROD LIMIT 1);
        SET STOCKACTUAL = (SELECT I.Stock FROM INVENTARIO_SUCURSAL AS I WHERE I.IdSucursal = IDSUC AND I.IdProducto = IDPROD AND I.Estatus = 1);

        UPDATE INVENTARIO_SUCURSAL AS I SET I.Stock = (I.Stock-PIEZASVAR) WHERE I.IdSucursal = IDSUC AND I.IdProducto = IDPROD AND I.Estatus = 1;

        SET PRODRESER = (SELECT I.Stock FROM INVENTARIO_SUCURSAL AS I WHERE I.IdProducto = IDPROD AND I.IdSucursal = IDSUC AND I.Estatus = 3);
        IF PRODRESER IS NULL THEN
            INSERT INTO INVENTARIO_SUCURSAL (IdProducto, IdSucursal, Stock, Estatus) VALUES (IDPROD, IDSUC, PIEZASVAR, 3);
        ELSE
            UPDATE INVENTARIO_SUCURSAL AS I SET I.Stock = (I.Stock+PIEZASVAR) WHERE I.IdSucursal = IDSUC AND I.IdProducto = IDPROD AND I.Estatus = 3;
        end if;

        INSERT INTO BITACORAINVENTARIO (Usuario, Accion, Sucursal, IdProducto, Campo, ValorAnterior, ValorNuevo) VALUES (USERID, 'UPDATE', IDSUC, IDPROD,'Stock', STOCKACTUAL, (STOCKACTUAL-PIEZASVAR));
...
CREATE PROCEDURE SP_ADDPEDIDO(IN IDNOTAIN INT, IN RECEPTORIN VARCHAR(100), IN IDPAGOIN INT, IN METPAGOIN VARCHAR(50), IN USERID INT, IN CREDITOIN DECIMAL(10,2), IN MONTOIN DECIMAL(60,30))
    BEGIN
        DECLARE CLIENTERANGOVAR INT;
        DECLARE CLIENTEVAR INT;
        DECLARE PEDIDOVAR INT;
        DECLARE SUCURSALVAR INT;
        DECLARE DEUDAVAR DECIMAL(65,30);

        SET SUCURSALVAR = (SELECT E.IdSucursal FROM USUARIO AS U INNER JOIN EMPLEADO E on U.IdEmpleado = E.IdEmpleado WHERE U.IdUsuario = USERID LIMIT 1);
        SET CLIENTERANGOVAR = (SELECT C.IdRangoCliente FROM CLIENTE AS C INNER JOIN NOTA N2 on C.IdCliente = N2.IdCliente WHERE N2.IdNota = IDNOTAIN LIMIT 1);

        SELECT FN_ACTUALIZARSTOCK(IDNOTAIN, NP.IdProducto, SUCURSALVAR, USERID) FROM NOTA_PRODUCTOS AS NP WHERE NP.IdNota = IDNOTAIN;
        ...
        SET PEDIDOVAR = (LAST_INSERT_ID());
        INSERT INTO BITACORA (Usuario, Accion, TablaAfectada, IdRegistro, Campo, ValorAnterior, ValorNuevo) VALUES (USERID, 'INSERT', 'PEDIDO', PEDIDOVAR, 'Todos', '', PEDIDOVAR);
    end;

DROP PROCEDURE IF EXISTS SP_GETNOTATICKET;
CREATE PROCEDURE SP_GETNOTATICKET(IN IDNOTAIN INT)
    BEGIN
        DECLARE CLIENTERANGOVAR INT;
        DECLARE USERVAR INT;
        DECLARE IDCLIENTEVAR INT;
        ...
        SELECT NUMPRODUCTOS AS NumeroProductos;
        CALL SP_CALCULARENVIO(IDCLIENTEVAR,USERVAR);
        SELECT CONCAT(P.Nombre, ' ', P.ApellidoPaterno, ' ', P.ApellidoMaterno) AS Nombre, (SELECT FN_OBTENERDIRECCION(C2.IdDireccion)) AS Direccion FROM NOTA AS N INNER JOIN CLIENTE C2 on N.IdCliente = C2.IdCliente INNER JOIN PERSONA P on C2.IdPersona = P.IdPersona WHERE N.IdNota = IDNOTAIN;
        SELECT S.Nombre, (SELECT FN_OBTENERDIRECCION(S.IdSucursal)) Direccion FROM NOTA AS N INNER JOIN EMPLEADO AS E ON N.IdEmpleado = E.IdEmpleado INNER JOIN SUCURSAL AS S ON E.IdSucursal = S.IdSucursal WHERE N.IdNota = IDNOTAIN AND E.IdEmpleado = N.IdEmpleado;
        SELECT CONCAT(P.Nombre, ' ', P.ApellidoPaterno, ' ', P.ApellidoMaterno) AS Nombre, R.Rol AS Direccion FROM NOTA AS N INNER JOIN EMPLEADO AS E ON N.IdEmpleado = E.IdEmpleado INNER JOIN PERSONA AS P ON E.IdPersona = P.IdPersona INNER JOIN ROL R on E.IdRol = R.IdRol WHERE N.IdEmpleado = E.IdEmpleado AND N.IdNota = IDNOTAIN;
        SELECT IDNOTAIN AS Nota;
    end;
...
DROP FUNCTION IF EXISTS FN_ACTUALIZARSTOCKSALIDA;
CREATE FUNCTION FN_ACTUALIZARSTOCKSALIDA(IDNOTAIN INT, IDPROD INT, IDSUC INT, USERID INT)
RETURNS VARCHAR(10)
    DETERMINISTIC
    BEGIN
        DECLARE STATUSVAR VARCHAR(10);
        ...
        SET PIEZASVAR = (SELECT Piezas FROM NOTA_PRODUCTOS WHERE IdNota = IDNOTAIN AND IdProducto = IDPROD LIMIT 1);
        SET STOCKACTUAL = (SELECT I.Stock FROM INVENTARIO_SUCURSAL AS I WHERE I.IdSucursal = IDSUC AND I.IdProducto = IDPROD AND I.Estatus = 3);

        UPDATE INVENTARIO_SUCURSAL AS I SET I.Stock = (I.Stock-PIEZASVAR) WHERE I.IdSucursal = IDSUC AND I.IdProducto = IDPROD AND I.Estatus = 3;

        INSERT INTO BITACORAINVENTARIO (Usuario, Accion, Sucursal, IdProducto, Campo, ValorAnterior, ValorNuevo) VALUES (USERID, 'UPDATE', IDSUC, IDPROD,'StockApartado', STOCKACTUAL, (STOCKACTUAL-PIEZASVAR));

        SET STATUSVAR = 'OK';
        RETURN STATUSVAR;
    end;

DROP FUNCTION IF EXISTS FN_ACTUALIZARSTOCKCANCELAR;
CREATE FUNCTION FN_ACTUALIZARSTOCKCANCELAR(IDNOTAIN INT, IDPROD INT, IDSUC INT, USERID INT)
RETURNS VARCHAR(10)
    DETERMINISTIC
    BEGIN
        DECLARE STATUSVAR VARCHAR(10);
...
CREATE PROCEDURE SP_ACTUALIZARPEDIDO(IN PEDIDOIN INT, IN ESTATUSIN VARCHAR(20), IN REPARTIDORIN INT, IN USERID INT)
    BEGIN
        ...
        SET SUCURSALVAR = (SELECT P.IdSucursal FROM PEDIDO AS P WHERE P.IdNota = PEDIDOIN LIMIT 1);
        ...
        IF ESTATUSIN = 'Enviado' THEN
            SELECT FN_ACTUALIZARSTOCKSALIDA(PEDIDOIN, NP.IdProducto, SUCURSALVAR, USERID) FROM NOTA_PRODUCTOS AS NP WHERE NP.IdNota = PEDIDOIN;
        end if;

        IF ESTATUSIN = 'Cancelado' THEN
            SELECT FN_ACTUALIZARSTOCKCANCELAR(PEDIDOIN, NP.IdProducto, SUCURSALVAR, USERID) FROM NOTA_PRODUCTOS AS NP WHERE NP.IdNota = PEDIDOIN;
        end if;
...
DROP PROCEDURE IF EXISTS SP_GETCLIENTESVENDEDORPAGE;
...
CALL SP_ADDCLIENTE(38940,49030,'Durán 76','Pedro','Garza','Martinez','4454672918','45',1,0,0,1001);
