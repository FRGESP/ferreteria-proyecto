...
CREATE PROCEDURE SP_GETNOTA(IN IDNOTAIN INT)
    BEGIN
        ...
        SELECT NP.IdNotaProducto, P.IdProducto, P.Descripcion, NP.Piezas, FORMAT((SELECT FN_GETPRECIO(P.IdProducto, CLIENTERANGOVAR)),2) AS PrecioUnitario, FORMAT(((SELECT FN_GETPRECIO(P.IdProducto, CLIENTERANGOVAR))*NP.Piezas),2) AS Importe FROM NOTA_PRODUCTOS AS NP INNER JOIN PRODUCTO AS P ON NP.IdProducto = P.IdProducto INNER JOIN NOTA N on NP.IdNota = N.IdNota WHERE NP.IdNota = IDNOTAIN;
...
CALL SP_ADDPRODUCTOVENTA(1,2,1001,2,1);
CALL SP_GETNOTA(2);
