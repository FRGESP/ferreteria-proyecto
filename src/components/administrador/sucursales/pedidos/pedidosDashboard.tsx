...
function PedidosDashboard({ IdSucursalProp }: PedidosDashboardProps) {
    ...
    const getPedidos = async () => {
        const data = await getPedidosAction(searchValue.pedido ? searchValue.pedido : "0", IdSucursalProp);
        setPedido(data);
        console.log(data);
    }
    ...
    return (
        <div className='w-full h-full flex flex-col items-center justify-center'>
            ...
            <div className='w-full flex flex-col items-center justify-center gap-3 '>
                {pedidos.map((pedido) => (
                    <div onClick={() => router.push(`/users/administrador/sucursales/${IdSucursalProp}/pedidos/${pedido.IdPedido}`)} key={pedido.IdPedido} className='flex flex-col gap-2 border border-black border-solid rounded-lg cursor-pointer hover:bg-gray-50 w-full'>
