...
function UpdateModalCargos({ IdCargo, onGuardado }: UpdateModalCargosProps) {
    ...
    const handleSubmit = async () => {
        ...
        if (Object.keys(newErrors).length == 0) {
            ...
            //     Campo: key,
            //     ValorAnterior: Cargo ? key == 'Tipo' ? Cargo['NombreTipo'] : Cargo[key as keyof Cargo] : "",
            //     ValorNuevo: value,
            // }));
            // console.log(registrosBitacoras);
            // console.log(inputValue);
            // const response = await updateBitacoraCargo(IdCargo, registrosBitacoras);
            // const responseUpdate = await axios.put(`/api/users/administrador/productos/categorias/${IdCargo}`, inputValue)
